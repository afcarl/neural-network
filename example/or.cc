#include <iostream>
#include "../perceptron.h"

void create_training_data(std::vector<std::vector<double> >& input_data,
std::vector<double>& output_data) {
	double input_arr[4][2] =
	{0, 0,
	 1, 0,
	 0, 1,
	 1, 1};
	double output_arr[4] = {0, 1, 1, 1};
	
	for (int i = 0; i < 4; i++) {
		input_data[i].assign(input_arr[i], input_arr[i] + sizeof(input_arr[i]) / sizeof(double));
		output_data[i] = output_arr[i];
	}
}

int main(void) {
	NeuralNetwork::Perceptron p(2);
	p.randomizeWeights(-1, 1);
	std::vector<std::vector<double> > input_data(4);
	std::vector<double> output_data(4);
	create_training_data(input_data, output_data);
	
	std::cout << "Untrained perceptron:" << std::endl;
	for (int i = 0; i < 4; i++) {
		std::cout << "input vector: " << input_data[i][0] << " "<< input_data[i][1] << ", ";
		std::cout << "expected output: " << output_data[i] << ", ";
		std::cout << "output: " << p.getOutput(input_data[i]) << std::endl;
	}
	
	int epochs = 100;
	
	p.trainOnData(input_data, output_data, epochs);
	
	std::cout << std::endl << "After " << epochs << " iterations:" << std::endl;
	for (int i = 0; i < 4; i++) {
		std::cout << "input vector: " << input_data[i][0] << " "<< input_data[i][1] << ", ";
		std::cout << "expected output: " << output_data[i] << ", ";
		std::cout << "output: " << p.getOutput(input_data[i]) << std::endl;
	}
	
	return 0;
}